...
On commence par télécharger les données puis on crée quelques variables utiles :
...
# Charger les données
print(setwd(Sys.getenv("path_bdd")))
donnees <- readRDS(file.path(Sys.getenv("path_bdd"), "base_PC_Var_X_Var_Y_ENSAI_Respi_FINALE.RData"))
print(donnees)
...
Puis on enregistre les variables avec des valeurs manquantes supérieures à 85 %.
```{r}
modalites_manquantes <- logical(n)

for (i in 1:n) {
    modalites_manquantes[i] <- sum(is.na(donnees[, i])) / num_individus > 0.85
}

if (!any(modalites_manquantes)) {
    cat("Il n'y a pas de données manquantes à plus de 85%.")
} else {
    cat("Des variables avec plus de 85% de données manquantes ont été détectées.")
}
```
...
# On exclut les variables avec peu de valeurs manquantes
