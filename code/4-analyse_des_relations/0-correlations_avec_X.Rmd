...
data <- readRDS("data_post_etape_4_Forest.Rdata", "rb")
data2 <- readRDS("data_post_etape_4_Mean.Rdata", "rb")
...
# Resultats Base Imputation Random Forest
```{r}
...
# Resultats Base Imputation Moyenne
```{r, include=F, eval=T}
data.explicatives2 <- data2[-c(1,23)]
```
```{r}
p_val_mat2 <- as.matrix(matrice_p_valeur(data.explicatives2))
visual_mat <- formatC(p_val_mat2, format = "e", digits = 2)
kable(visual_mat, format = "markdown")

```
```{r, fig.width=10, fig.height=10, fig.fullwidth=TRUE}
heatmap_with_border(p_val_mat2, "Matrice des p-valeurs", threshold = 0.05)
```
...
```{r, eval=T, include=F}
# On regarde qu'on a bien les bonnes variables dans le df des variables et corrélation avec y
all(df_var$var == colnames(p_val_mat2))
```
```{r}
# Création de a matrice indicant si deux variables sont correlées:
p_val_mat_bool2 <- p_val_mat2 <= 0.05
# Création d'une colonne qui compte le nombre de variables corrélées
df_var$nb_var_corr <- NA
for (i in 1:ncol(p_val_mat2)){
  df_var$nb_var_corr[i] <- sum(as.numeric(p_val_mat_bool2[i,])) - 1
}
summary(df_var)
```
...
colnames(df_var) <- c("variable", "p_value", "nb_var_corr" )
variables_origi <- df_var$variable
df_var_minimized2 <- minimize_p_value_sum(df_var, p_val_mat_bool2)
variables_gar2 <- df_var_minimized2$opt_df
# Vraiables à supprimmer
setdiff(variables_origi, variables_gar2$variable)
# variables à garder
variables_gar2$variable
```
On garde alors un total de 14 variables.

# Blocs variables

En rouge, les variables supprimées ; en orange, celles supprimées dans l'une des méthodes d'imputation ; et en bleu, les variables absentes dans l'autre base imputée.

## Sans suppression de variables

| Logement & Entretien  | Alimentation               | Biosécurité             | Gestion des maladies | Caractéristiques Générales | Facteurs Infectieux      |
|----------------------|---------------------------|-------------------------|----------------------|----------------------------|--------------------------|
| X22x1_LOC_INF_rec   | X09x3_FAB_CROISS_reg_rec  | X12x2_MAT_PC           | **<span style="color:red">X18x2_ABBTX_PS</span>**      | X06x1_gene_majo_1_rec      | A03_Pos10sVERS           |
| X07x1_AN_CONST4_mean_3 | T10_PS_AlimPoNourrLongpPo |  **<span style="color:orange">X12x2_MAT1FRLAV_1</span>**  |    **<span style="color:orange">X19x2_ABB_E </span>**      | **<span style="color:red">X25x1_ElvAlterChoiPers</span>**     | **<span style="color:orange">A03_PosSeroMyAs</span>**          |
| ENG_TROU            |  T10_PS_EauDebi_3 | T16_BS_TenueSpeciElev  |                      | **<span style="color:red">LR_LRF</span>**                     | **<span style="color:red">A03_TxPosSero22sTgReel</span>**      |
|  **<span style="color:orange">T13_ENG_milieuDegrad.x</span>** |                           |    **<span style="color:red">X13x2_QUAI_t </span>**      |                      |                            | A04_My10Mfloc            |
|                      |                           |                         |                      |                            |  A05_TxPos10sGRIPPEReel  |
...
| Logement & Entretien  | Alimentation               | Biosécurité             | Gestion des maladies | Caractéristiques Générales | Facteurs Infectieux      |
|----------------------|---------------------------|-------------------------|----------------------|----------------------------|--------------------------|
| X22x1_LOC_INF_rec   | X09x3_FAB_CROISS_reg_rec  | X12x2_MAT_PC           | **<span style="color:blue">X19x2_ABB_E</span>**          | X06x1_gene_majo_1_rec      | A03_Pos10sVERS           |
| X07x1_AN_CONST4_mean_3 | T10_PS_AlimPoNourrLongpPo | T16_BS_TenueSpeciElev  |                      |                            | **<span style="color:blue">A03_PosSeroMyAs</span>**           |
| ENG_TROU            | T10_PS_EauDebi_3          |                        |                      |                            | A04_My10Mfloc            |
|                      |                            |                         |                      |                            | A05_TxPos10sGRIPPEReel   |
...
| Logement & Entretien | Alimentation | Biosécurité | Gestion des maladies | Caractéristiques Générales | Facteurs Infectieux |
|----------------------|-------------|------------|----------------------|----------------------------|----------------------|
| X22x1_LOC_INF_rec   | X09x3_FAB_CROISS_reg_rec | X12x2_MAT_PC |  | X06x1_gene_majo_1_rec | A03_Pos10sVERS |
| X07x1_AN_CONST4_mean_3 | T10_PS_AlimPoNourrLongpPo |  **<span style="color:blue">X12x2_MAT1FRLAV_1</span>**  | | | A04_My10Mfloc |
| ENG_TROU | T10_PS_EauDebi_3 | T16_BS_TenueSpeciElev | | | A05_TxPos10sGRIPPEReel |
|  **<span style="color:blue">T13_ENG_milieuDegrad.x</span>** |  | | | | |


# Differences resultats 

Les variables supplémentaires dans la base issue de la forêt aléatoire sont :
```{r}
setdiff(variables_gar$variable, variables_gar2$variable)
```
* Antibiotique en cas de besoin
* moyenne positivité sup à 0,5

Et celles de la base issue de l'imputation par la moyenne sont :
```{r}
setdiff(variables_gar2$variable, variables_gar$variable)
```
* Fréquence lavage matériel
* Milieu dégradé en engraissement (sol sale et / ou humide)

Dans la base issue de la méthode d'imputation par la forêt aléatoire, on garde deux variables supplémentaires liées à l'état sanitaire des porcs, tandis que dans celle issue de l'imputation par la moyenne, on conserve deux variables décrivant les conditions sanitaires des élevages.