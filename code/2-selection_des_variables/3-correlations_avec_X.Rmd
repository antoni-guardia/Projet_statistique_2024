...
setwd("/home/rogerbernat/Documents/ENSAI/ensai 2024-25/Projet_statistique_2024")
...
data.explicatives <- data[-c(64)]
names(data.explicatives)
```
...
```{r}
names(data.explicatives)
p_val_mat <- as.matrix(matrice_p_valeur(na.omit(data.explicatives)))
p_val_mat
...
heatmap_with_border <- function(mat, titre="", threshold = 0.05, margin = c(14, 14, 4, 2)) {
  ...
  rect(0.45, -0.1, 0.55, -0.15, col = "red", border = NA)
  text(0.5, -0.125, "p < 0.05", col = "white", cex = 0.8)
  
  axis(1, at = seq(1, 100, length.out = 5), labels = round(seq(min_val, max_val, length.out = 5), 2), lwd = 0.05)
}
png("matrice_correlations_63.png", height=1200, width=1200)
heatmap_with_border(p_val_mat, "", threshold = 0.05)
dev.off()
